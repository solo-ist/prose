...
- renders the **comment rail** as an absolutely-positioned margin column with a stacking engine: open cards align to their highlight (mark top − 6px) and push down on collision; collapsed Resolved and "Lost their place" sections and the compose form stack in flow; re-layout is debounced and re-fires on fonts/resize/merge/toggle.
...
- detects mode by protocol — **`file://` → full in-file commenting** (a top banner says the copy is local; additions bake into a downloadable annotated copy that Prose re-imports — the sneakernet loop); `https://` + shareEndpoint → online commenting, token read from the `/s/<token>` path, never embedded;
...
- **polls the live conversation** (`GET /s/:token/comments` on load, every 45s, on window focus, ~2s after a post — same-origin, so CSP `connect-src 'self'` holds): the merge is **additive-only** (never deletes local entries → offline and just-posted additions survive), dedupes threads and replies **by id**, and treats the poll as authoritative only for the `resolvedAt` of rows it returns; a 410 stops polling, swaps the rail note to the takedown copy and closes every commenting entry point while the page stays readable. Author replies (`fromAuthor`, or nameless baked replies) get an "· author" tag;
...
- **Pull** (reviewer → Prose): `share:pullComments` fetches since the stored cursor **without advancing it**; the renderer merges + persists, then acks (`share:ackCursor`) — a failed merge can never lose comments. Triggers: window focus (≥ 5-min debounce per share) + opening the ◎ popover. The renderer reload (`loadComments` → `needsRestore` → `restoreComments`) derives anchors; unmatched threads are **flagged `anchorLost: true` and KEPT**.
- **Push** (Prose → reviewers): author replies and resolves POST up immediately via `lib/sharePush.ts`, fire-and-forget, hooked ONLY into the user/AI action handlers (CommentPopover, Comment Review, the AI tool executors) — **never into `saveComments`**, which also runs on tab flushes and the sync merge itself and would echo-loop. Failures queue and flush on focus / after content pushes.
- **Reply dedupe invariant:** a pushed reply records its gateway row id as `CommentReply.shareId`. The pull-merge treats a local reply's `id` AND `shareId` as seen; artifact bakes emit the reply **under `shareId`** (and never embed the field) — so one reply is one id everywhere: desktop store, baked page, live poll.
