...
- **Origin isolation (#902):** artifacts are author-controlled HTML+JS by design, so with `SHARE_BASE_URL` set, `/s/*` serves from a dedicated **cookie-less share host** — the share host answers ONLY `/s/*` (+ health), so artifact JS finds no API, no session, and no shared localStorage with the API origin (which closes cross-share edit-token/draft theft, cross-author session riding, and on-domain phishing). The API host 308-redirects artifact *page* loads; the JSON comment sub-routes stay dual-host so pre-split file:// copies keep publishing (capability-gated, cookie-free, nothing executes). The viewer derives its API base from `location.origin` (always `connect-src 'self'`-clean) and annotated copies bake the serving origin. Unset = single-origin dev default; **production requires the share domain configured in Render + DNS before `share_publish` widens.**
