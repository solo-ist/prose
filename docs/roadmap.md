...
**Status:** Active · **Updated:** 2026-09-04 · **Live priority:** [Project board #5](https://github.com/orgs/solo-ist/projects/5/views/1)
...
- **Next / now spinning up: Wave 1 — Core Build-Out** — three concurrent tracks on the **A→B→C merge ladder**. **Track C (Paid Platform, [#598](https://github.com/solo-ist/prose/issues/598)) is the first to move — Phase 0 now Do First:** the stack was selected 2026-06-15 and infra locked 2026-06-30 (Hono + Postgres + **Prisma 7** monolith on **Render**, DNS on **Hover**; Cloudflare = **R2** blobs only · **Better Auth** self-hostable · no-CRDT comment sync; planning doc [`architecture/web-platform.md`](architecture/web-platform.md)). The epic decomposed into **three discovery spikes** ([#601](https://github.com/solo-ist/prose/issues/601) auth/CSRF · [#775](https://github.com/solo-ist/prose/issues/775) SSE-through-Hono · [#776](https://github.com/solo-ist/prose/issues/776) comment-sync design) plus **seven build phases** ([#765–#771](https://github.com/solo-ist/prose/issues/765)). **All three spikes are now resolved** — #601 (Better Auth) · #775 (SSE-through-Hono, 15/15) · **#776 (2026-08-27 → the embedded-artifact comment layer: comments travel inside the share HTML, gateway = thin relay, no-CRDT pure-data-merge sync; reshaped #768/#769 and de-gated #768 from #767)** — so **[#765](https://github.com/solo-ist/prose/issues/765) Phase 0 is Do First** (gateway deployed to Render, A.5 go/no-go passed) and #766–#771 are queued. #602 resolved (at-cost + entitlements); #258 folded in. **New direction docs (2026-08-27):** [`architecture/atproto-publishing.md`](architecture/atproto-publishing.md) (self-hosted PDS + Standard.site/Markpub lexicons, deferred/gated) and [`architecture/operating-model.md`](architecture/operating-model.md) (the co-op mechanics answer). Epic now **12 sub-issues** (deferred #439 + resolved #602 attached for traceability). Track B stays scope-gated by [#616](https://github.com/solo-ist/prose/issues/616) (closeout recommendation posted, pending ratification).
...
- **Column = phase, milestone = wave:**
  | Column | Phase |
  |---|---|
  | **Do First** + **Quick Wins** | priority/bigger vs. small — the v1.6.5 desktop batch shipped; **Do First** now leads with **Track C Phase 0 ([#765](https://github.com/solo-ist/prose/issues/765) gateway scaffold)** — all three spikes (#601/#775/#776) resolved — plus #487 (MAS provisioning), **Quick Wins** holds the v1.6.5 follow-ups (#790/#798/#801) alongside #536/#384 |
  | **Do Next** | Wave 0.5's human (ASC) tail + Wave 1 Track C build phases (#766–#771, epic #598) + #603 + #799 |
...
**Payment philosophy:** a **co-op**, not a profit center — at-cost, likely usage-based plus a small monthly shared-infra subsidy. The subscription is a convenience layer over LLM access. **Mechanics resolved 2026-08-27** — the "public utility posture" (published unit economics · monthly cost-ledger post · governance thresholds; no legal entity at v1): [`architecture/operating-model.md`](architecture/operating-model.md).
...
| [#776](https://github.com/solo-ist/prose/issues/776) | **Comment-sync design.** **Resolved 2026-08-27 → the embedded-artifact model** (comments inside the share HTML + inline viewer; gateway = thin relay; pure-data-merge sync, anchoring stays in `restoreComments`; reconciles #699). Reshaped #768/#769 — see [`architecture/web-platform.md`](architecture/web-platform.md) §4.3–4.5. |
...
**Track C — Paid Platform Foundation** ([#598](https://github.com/solo-ist/prose/issues/598)) — *stack selected 2026-06-15, infra locked 2026-06-30, comment-layer design locked 2026-08-27; decomposed into native sub-issues (**12**: 3 spikes + 7 phases + attached #439/#602). All three spikes resolved (#601/#775/#776) → **[#765](https://github.com/solo-ist/prose/issues/765) Phase 0 is Do First**; #766–#771 are Do Next.* **Stack:** Hono + Postgres (**Prisma 7**) monolith on **Render** · DNS on **Hover** (Render-managed TLS) · Cloudflare **R2** for blobs only (docs live in Postgres) · **Better Auth** (self-hostable) · no-CRDT comment sync via the **embedded-artifact model** (#776). **Planning doc:** [`architecture/web-platform.md`](architecture/web-platform.md). Critical path: #765 → #766 → #768 → #769 — **#768 is de-gated from #767** (the share artifact is self-contained), so #767 runs parallel off #766, alongside #770; #771 cross-cutting.
| Issue | Captures |
|---|---|
| [#765](https://github.com/solo-ist/prose/issues/765) | Phase 0 — gateway scaffold + deploy (Hono/PG/Prisma 7/R2; integrates #601 auth + #775 SSE). |
...
| [#768](https://github.com/solo-ist/prose/issues/768) | Phase 3a — **share artifact + embedded viewer + one-way comments**: self-contained HTML (comments + inline viewer inside the file), hashed-token serving, anonymous reviewers, ShareDialog, `webPlatform` flag. **De-gated from #767** per #776's design. |
| [#769](https://github.com/solo-ist/prose/issues/769) | Phase 3b — **bidirectional comment sync**: pure data merge in `src/main/share/`, `anchorLost` kept + surfaced, re-publish bake-in, resolution-to-history. Implements the #776 design. |
...
| [#598](https://github.com/solo-ist/prose/issues/598) Paid Platform Foundation | Track C / Wave 1 — **active (Phase 0 Do First)** | Wave 1 | all spikes resolved (#601/#775/#776); #765 (Do First) + #766–#771 (reshaped #768/#769); #364; #258 folded; attached children #439 (hosted-OCR) + #602 (billing, resolved) — **12 sub-issues, 3 done** |
...
- ~~**Co-op mechanics**~~ — **resolved (2026-08-27):** solo benevolent operator + published at-cost unit economics + a monthly public cost-ledger post; community-input tier at ~20–50 paying members; formal co-op structure evaluated only past ~100. See [`architecture/operating-model.md`](architecture/operating-model.md).
...
- **ATProto document distribution (deferred, direction locked):** [`architecture/atproto-publishing.md`](architecture/atproto-publishing.md).
- **Web operating model (co-op mechanics):** [`architecture/operating-model.md`](architecture/operating-model.md).
